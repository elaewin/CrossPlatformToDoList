...
typedef void(^FirebaseCompletion)(NSArray *allTodos);
...
// Helper method to monitor changes on Firebase
-(void)startMonitoringTodoUpdates:(FirebaseCompletion)completion {
    ...
    [[userReference child:@"todos"] observeEventType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            completion(todoDictionaries.copy);
        });
...
-(void)session:(WCSession *)session didReceiveMessage:(NSDictionary<NSString *,id> *)message replyHandler:(void (^)(NSDictionary<NSString *,id> * _Nonnull))replyHandler {
    ...
    // sending the array of todos from Firebase to the watch.
...
// pragma MARK: Unused boilerplate methods
